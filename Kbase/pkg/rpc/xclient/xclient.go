...
func (xc *XClient) Close() error {
	...
	for key, client := range xc.clients {
		// Ihave no idea how to deal with error, just ignore it.
		_ = client.Close()
...
func (xc *XClient) Broadcast(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	...
	var e error
	replyDone := reply == nil // if reply is nil, dont't need to set value
	...
	for _, rpcAddr := range servers {
		...
		go func(rpcAddr string) {
			...
			if err != nil && e == nil {
				e = err
				cancel() // if an call failed, cancel unfinished calls
			}
